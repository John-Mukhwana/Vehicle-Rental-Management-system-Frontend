...
import Loading from './loading'; // Ensure you have the Loading component created as shown before
...
const UserBookings: React.FC = () => {
      ...
      try {
        ...
        if (Array.isArray(response.data)) {
          const userBookings = response.data.filter(
            (booking: Booking) => booking.userId === userId && booking.bookingStatus !== 'Cancelled'
          );
          ...
          const todayBookings = userBookings.filter(
            booking => new Date(booking.bookingDate).getTime() >= startOfToday && new Date(booking.bookingDate).getTime() < endOfToday
          );
          ...
          // Combine today's bookings and pending bookings, ensuring no duplicates
          const recentBookings = [...new Set([...todayBookings, ...pendingBookings])];
  ...
  return (
    <div className="p-6 bg-gray-100 dark:bg-gray-900 min-h-screen">
      <h2 className="text-2xl font-bold mb-6 text-gray-800 dark:text-gray-200">Recent Bookings</h2>
      ...
      {loading ? (
        ...
        bookings.length > 0 ? (
          ...
          <p className="text-center text-gray-600 dark:text-gray-400">No recent bookings found</p>
        )
      )}
    </div>
  );
